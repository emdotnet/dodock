...
frappe.views.Workspace = class Workspace {
	...
	async setup_pages(reload) {
		...
		this.all_pages.forEach((page) => {
			page.is_editable =
				(!page.is_standard && !page.public) ||
				(!page.is_standard && this.has_access) ||
	...
	sidebar_item_container(item) {
		const iconAttributes = Object.entries({
			...
			.join(" ");
		const iconHtml = frappe.utils.icon(item.icon || "folder-normal", "md");
	...
	append_item(item, container) {
		let is_current_page =
			frappe.router.slug(item.name) == frappe.router.slug(this.get_page_to_show().name) &&
	...
	merge_extended_item(item_type, default_columns = 4) {
		const extended_shortcuts = this.page_data[`${item_type}s`]
			? this.page_data[`${item_type}s`].items.filter((f) => f.extension)
			: [];
		if (extended_shortcuts.length) {
			const existing_cards = this.content
				.filter((f) => f.type == item_type)
				.map((f) => f.data)
